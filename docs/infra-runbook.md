# 🏢 MediMaven Infrastructure Runbook
...
1.  [Accounts / Regions](#accounts--regions)
2.  [Compute Layer](#compute-layer)
3.  [Networking & Security](#networking--security)
4.  [Application Load Balancer](#application-load-balancer)
5.  [DNS & TLS](#dns--tls)
6.  [Start / Stop Automation](#start--stop-automation)
7.  [Logging & Monitoring](#logging--monitoring)
8.  [Re-deployment Steps](#re-deployment-steps)
...
| EBS | **gp3 50 GiB** @ 3 000 IOPS mounted **/mnt/gp3disk** – `UUID=… /mnt/gp3disk ext4 defaults,nofail 0 2` |
| Docker | Engine enabled @ boot `sudo systemctl enable docker` |
| Compose file | `~/gp3disk/Medimaven/docker‑compose.prod.yml` |
| Containers | `Medimaven-rag-api` (port 8000) with `restart: always` |
...
|-----------|--------------------|-------------------------|---------------|-----------|
| Images | phade160/medimaven-rag-api | Docker Hub + GHCR | `docker pull …` | Already public |
| Compose stack | docker-compose.prod.yml | EC2 | `docker compose up -d` | restart:always |
| Data disk | 50 GiB gp3 `/mnt/gp3disk` | EBS | create → attach → fstab | \$2.40 / mo |
...
| Budget | *(todo)* \$100 | AWS Budgets | billing console | email alert |
