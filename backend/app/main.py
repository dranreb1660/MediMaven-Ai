...
class ConversationManager:
    ...
    async def append_turn_optimized(
        ...
        """Simplified append with better error handling."""
        user_id = user["sub"]
        turn_with_meta = {
            **turn,
        ...
        try:
            ...
            logger.debug(f"✅ Turn appended for cid={cid}")
            
        except Exception as e:
            logger.error(f"History management failed: {e}")
            # Simple fallback without complex retries
    ...
    async def _handle_new_conversation(self, user_id: str, cid: str, turn: Dict[str, Any]):
        """Handle new conversation with cleanup."""
        try:
            ...
            await self.db.execute(
                cleanup_query, 
                {"user_id": user_id, "max_threads": MAX_THREADS_PER_USER - 1}
            )
            ...
            logger.error(f"New conversation creation failed: {e}")
            raise
    
    async def _append_to_existing_conversation(self, cid: str, turn: Dict[str, Any]):
        """Append to existing conversation with turn limit."""
        try:
            ...
            if not conversation:
                logger.warning(f"Conversation {cid} not found")
                return
            ...
            await self.db.execute(
                ...
                .values(messages=updated_messages, updated_at=func.now())
            )
    ...
    async def _simple_append_fallback(self, user_id: str, cid: str, turn: Dict[str, Any], first: bool):
        ...
        try:
            ...
            await self.db.execute(stmt, {"one": [turn]} if not first else {})
            await self.db.commit()
    ...
    try:
        ...
        if config.REDIS_URL:
            try:
                ...
                rc = redis.from_url(config.REDIS_URL, socket_timeout=5, socket_connect_timeout=5)
                await rc.ping()
...
async def run_rag(req: ChatRequest) -> tuple:
    ...
    try:
        ...
        # 3) Rewrite follow-up if needed (KEEP ORIGINAL LOGIC)
        try:
...
async def chat_stream(
    ...
    try:
        ...
        # Prepare data (KEEP ORIGINAL LOGIC)
        query = sanitize_query(req.query)
        cid = req.conversation_id or str(uuid.uuid4())
        
        validated_history = validate_history(req.history or [])
        print(f'-------validated history:------------------\n {validated_history}')  # Debugging line
        mem = {"turns": validated_history}
        first = not mem["turns"] or not mem["turns"][0].get("assistant")
        ...
        async def events() -> AsyncGenerator[str, None]:
            ...
            try:
                ...
                # Stream tokens (KEEP ORIGINAL LOGIC)
                ...
                # Finalize response (KEEP ORIGINAL POSTPROCESSING)
